...
Everything in detail. For the overview, see the [README](../README.md).

- [Connect a model](#connect-a-model): providers, your own GPU, colibri and Soup, engine modes
- [Use it](#use-it): shortcuts, every command, working while it works, editors, worktrees, images, modes
- [How it thinks](#how-it-thinks), [Watch it think](#watch-it-think), [It learns](#it-learns)
- [Configuration](#configuration): settings, context, cost and budgets, several models, lean mode, hooks, MCP
- [Light on your machine](#light-on-your-machine), [Development](#development), [Roadmap](#roadmap)
...
<img src="images/home.png" alt="The start screen with the / command menu open" width="820">
...
- **Plans and modes:** the plan and the mode switcher (manual, edit, ask, plan, auto) work.
- **Stop:** stops the turn.
...
**Shift+Tab** cycles **manual → edit → ask → plan → auto**. Each mode has its own color in the status line,
and a switch while it works applies from the agent's next action.
...
| **ask** (cyan) | Talks the idea through: questions, options, trade-offs. It may read and search, but changes and runs nothing. |
| **plan** (green) | Read-only: the agent explores, then shows a plan. You choose to build it in auto, edit or manual mode, or to keep planning. |
| **auto** (yellow) | Runs on its own and says so under each step. Still asks for risky actions: `git push`, `git reset --hard`, publishing, `sudo`, piping downloads into a shell, and edits outside the project. |
| bypassPermissions | No checks at all. Only with `--mode bypassPermissions`, for throwaway environments. |

**A new folder is trusted first.** The first time you open a folder, MUYAH-CODE asks before anything from
it can load, because a folder's own hooks and MCP servers can run commands.

<img src="images/trust.png" alt="The trust check for a new folder" width="820">

**Deletes are never run by the agent, in any mode** (bypass included). It shows you the command and what it
would remove, puts it on your clipboard, and carries on without it.

<img src="images/permission.png" alt="A permission prompt showing the exact diff before an edit" width="820">
...
<p align="center">
  <img src="images/live-view.webp" alt="Live view of a two-turn session: a skill, an MCP call, two sub-agents, approvals, tests written first, a message queued while it works, live charts" width="900">
</p>
...
```bash
pip install -e ".[dev]"
python -m pytest -q          # parser, tools, permissions, context, learning, hooks, the full agent loop
...
- `scripts/`: `make_screenshots.py`, `record_cli_demo.py` and `record_viz_gif.py` make the README images and
  videos from the real UI; `bench_resources.py` measures RAM and CPU.
